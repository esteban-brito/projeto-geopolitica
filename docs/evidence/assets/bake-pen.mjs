/* A CANETA: a imagem dele (tmp/caneta-origem.png, 2172x724, alfa), cortada pela tinta e assada a
   420px — 210px de layout na mesa, dpr 2 —, nitidez raio 1 ganho 1,0, q 0,90. A mesma receita dos
   envelopes. uso: node tmp/assar-caneta.mjs */
...
const r = await page.evaluate(async src => {
  ...
  const W = 420, H = Math.round(ch * W / cw);
  ...
  for (let p = 0; p < W * H; p++) { if (d[p * 4 + 3] < 8) continue; for (let c = 0; c < 3; c++) { const i = p * 4 + c; d[i] = Math.max(0, Math.min(255, d[i] + 1.0 * (d[i] - blur[i]))); } }
  cx.putImageData(im, 0, 0);
  const blob = await cv.convertToBlob({ type: "image/webp", quality: 0.9 });
  const buf = new Uint8Array(await blob.arrayBuffer());
  let s = ""; for (const b of buf) s += String.fromCharCode(b);
  return { b64: btoa(s), w: W, h: H, bytes: buf.length, tinta: [cw, ch] };
}, src);
writeFileSync("assets/pen.webp", Buffer.from(r.b64, "base64"));
console.log(`assets/pen.webp: ${r.w}x${r.h}, ${(r.bytes / 1024).toFixed(0)} KB, tinta ${r.tinta.join("x")}`);
await browser.close();
